...
class EditMovie extends StatefulWidget {
  final MyMovie? params;

  const EditMovie({Key? key, this.params}) : super(key: key);
...
class _EditMovieState extends State<EditMovie> {
  ...
  void initialData() async {
    titleText.text = widget.params!.name;
    categoryText.text = widget.params!.category;
    descriptionText.text = widget.params!.author;
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          if (_formKey.currentState!.validate()) {
            ...
            _movieBloc?.add(UpdateMovieEvent(
                params: MovieParams(
                    id: widget.params!.id,
  ...
  BlocListener editMovieListener() {
    return BlocListener<MovieBloc, MoviesState>(listener: (context, state) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();

      if (state is IsLoadingUpdateMovie) {
        ProgressDialog.show(context);
        // showGeneralLoading(context);
      } else if (state is SuccessUpdateMovie) {
        ProgressDialog.dissmiss1(context);
        snackbarSuccess(context, "Actualizado correctamente");
        _movieBloc?.add(const GetAllMoviesByTitleEvent(title: ""));

        Navigator.pop(context);
      }
      if (state is ErrorUpdateMovie) {
        ProgressDialog.dissmiss1(context);
